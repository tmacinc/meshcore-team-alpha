...
- fix user display on map. Should be arrow with compass heading from device.

- push waypoint edits to other users that already have the waypoints - DONE

- historical track when user selected on map

- click user on map allows send message to user.
...
- Fix delayed opening of sync window.

- Revist heard notifications
...
- auto send message on reconnect? Currently gets lost.
...
- Test/add companion gps data functionality.
